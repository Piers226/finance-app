    ...
    <Container sx={{ mt: 4}}>
      <Box sx={{ 
        ...
        <Button
          variant="outlined"
          onClick={() => signOut()}
          sx={{ borderRadius: 6, bgcolor: "custom.logout", color: "rgb(255, 255, 255)" }}
        >
          Log out
        </Button>
      </Box>
        ...
        <Grid container spacing={2}>
          {categorySummaries.map(({ id, name, spent, targetAmount, percent }) => (
            <Grid item xs={12} sm={6} md={4} key={id}>
              <Box sx={{ p: 2, border: '1px solid #ddd', borderRadius: 8, backgroundColor: "primary.light" }}>
      ...
      >
        <Typography variant="h6">
          {viewBudgetLeft >= 0
            ...
            : `Over Budget by: $${Math.abs(viewBudgetLeft).toFixed(2)} (Budget: $${totalBudget.toFixed(2)})`}
        </Typography>
      </Box>
      <Divider sx={{ my: 3 }} />
      <Button
